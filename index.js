...
if (fileName.length < 1) {
	console.log('Help:');
	console.log(
		'Input "testLinks -v/--version" in command line --- Check the command line tool version and name!',
	);
	console.log(
		'Input "testLinks [file name]" in command line --- finding and reporting dead links!',
	);
	console.log(
		'Input "testLinks -j/--json [file name]" in command line --- output Json data!',
	);
	console.log(
		'Input "testLinks -i/--ignore [ignore file name] [file name]" in command line --- finding and reporting dead links excluding ignore urls!',
	);
	process.exit(1);
} else {
	if (
		fileName.length == 1 &&
		(fileName[0] == '-v' || fileName[0] == '--version')
	) {
		...
		if (
			(fileName[0] == '-j' || fileName[0] == '--json' || fileName[0] == 'j') &&
			fileName.length > 1
		) {
			flagj = true;
			fileName = fileName.slice(1);
